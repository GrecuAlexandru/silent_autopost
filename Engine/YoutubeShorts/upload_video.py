...
def write_output(text):
    print(text)
    outputtxt_path = os.path.abspath(
        os.path.join(os.path.dirname(__file__), "../../App/output.txt")
    )
...
def resumable_upload(insert_request):
    ...
    while response is None:
        ...
        if error is not None:
            ...
            write_output("Sleeping %f seconds and then retrying..." % sleep_seconds)
...
if __name__ == "__main__":
    ...
    try:
        ...
        write_output("An HTTP error %d occurred:\n%s" % (e.resp.status, e.content))
